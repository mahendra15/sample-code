...
{
		if(md5(trim($_POST['captcha_image']))==$_SESSION['captcha_image'])
	    {		
			

	         require_once('Rmail.php');
		    $mail = new Rmail();
			$mail->setFrom('Info <info@nagpurlife.com>');
			$mail->setPriority('high');
			$mail->setSubject('Enquiry for Nagpurlife');
			$mail->setBcc('ajaykumar@com1uk.com');
			$mail->setBcc('monish@com1uk.com');
			...
			$result  = $mail->send(array('gaz.summer@com1uk.com'));
	?>
	<fieldset>
        <div class="" style="width:500px;height:40px;">
            <p style="margin-left:20px;margin-top:5px;margin-bottom:5px;color:green;">
...
?>
    ...
    <fieldset>
        <div class="" style="margin-left:20px;">
            ...
            <input name="company" type="text" id="company" value='<?=$_POST['company']?>' />
            <label class="clear"><strong>Telephone Number</strong></label>
            <input name="txt_phone" type="text" id="txt_phone" value='<?=$_POST['txt_phone']?>' />
            <label class="clear"><strong>Your Message </strong><span style="color:#999;" id="mreq">(required)</span></label> 
            <textarea name="message" id="message" cols="45" rows="4"><?=$_POST['message']?></textarea>
            <label class="clear"><strong>Type the code shown </strong><span style="color:#999;" id="creq">(required)</span></label>
            <input name="captcha_image" type="text" id="captcha_image" />&nbsp;<img src="captcha_image.php" border="0" style="margin-top:5px;" />
    		 <div class="clear"></div>
            <input type="button" class="subscribeBtn" name="submitBtn" value="Submit" style="margin-left:0px;" onclick="return chk_booking(document.form, '<?php echo trim($_SESSION['captcha_image']); ?>');" />
   ...
   <?php
	...
	if($_POST && md5(trim($_POST['captcha_image']))!=$_SESSION['captcha_image'])
	{
	  ?> <script>document.getElementById("creq").innerHTML = "<font color='red'>(required)</font>"; document.getElementById("captcha_image").focus(); </script> <?php 
	}
	else
	{
	 ?> <script>document.getElementById("creq").innerHTML = "(required)"; </script> <?php 
	}
?>
...
<div id="footer">
  <div style="margin-left:20px; float:left;">&copy; <?php echo date("Y");?> nagpurlife.com, All rights reserved</div>
